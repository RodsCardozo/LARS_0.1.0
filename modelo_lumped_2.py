...
def lumped(calor, fg, area_rad):
    ...
    sig = 5.67e-8
    cores = ['#5F9EA0', '#66CDAA', '#7FFFD4', '#006400', '#556B2F', '#8FBC8F', '#2E8B57', '#3CB371', '#20B2AA',
             '#98FB98',
             '#00FF7F', '#7CFC00', '#00FF00', '#7FFF00', '#00FA9A', '#ADFF2F', '#32CD32', '#9ACD32', '#228B22',
             '#6B8E23',
             '#BDB76B']
    total = len(calor[0])
    cont = 0
    ...
    while n < numero_orbitas:
        ...
        with open(f'temperaturas_transiente_2/resultado_temp_{n}.csv', 'w', newline='') as arquivo_csv:
            writer = csv.writer(arquivo_csv)
            writer.writerow(
                ['Face1', 'Face2', 'Face3', 'Face4', 'Face5', 'Face6'])  # Escreve o cabeçalho do CSV, se necessário
